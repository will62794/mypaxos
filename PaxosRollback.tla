...
\* TODO: Under what assumptions on rollback behavior is the protocol still safe?
SomeAcceptors == CHOOSE q \in SUBSET Acceptor : Cardinality(q) = 1
RollbackAssumptions(a) == a \in SomeAcceptors

\* State of acceptor 'a' gets rolled back.
Rollback(a) == 
    /\ RollbackAssumptions(a)
    /\ maxBal' = [maxBal EXCEPT ![a] = -1] 
    /\ maxVBal' = [maxVBal EXCEPT ![a] = -1] 
    /\ maxVal' = [maxVal EXCEPT ![a] = None]
    /\ UNCHANGED msgs
